// Ghostty cursor-trail shader -- comet-like glow trail behind the cursor.
//
// Source: sahaj-b/ghostty-cursor-shaders, cursor_tail.glsl
//   https://github.com/sahaj-b/ghostty-cursor-shaders
// License: MIT (c) 2026 Sahaj Bhatt -- see that repo's LICENSE. Kept
// verbatim below except for this header; unmodified otherwise.
//
// TRAIL_COLOR is left at its upstream default (iCurrentCursorColor), which is
// Ghostty's own cursor colour -- already matugen's `primary` hue via
// [templates.ghostty] cursor-color = {{colors.primary.default.hex}} in
// ~/.config/matugen/config.toml, and re-themed on every wallpaper switch with
// no shader edit needed. Toggle: see shaders/cursor-trail.conf next to this
// file, included (optionally) from config.ghostty.
