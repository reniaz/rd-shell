...
temperature, opening a tabbed panel (Processor / Graphics / Memory) with
per-core detail and a process killer (SIGTERM/SIGKILL).
...
The Claude and system panels have no bind of their own — open them from
their pills, or bind `qs ipc -c rd-shell call claude toggle` /
`... call system toggle` yourself. Every bind above lives in the compositor
(`hypr/hyprland.lua`), not in the bar.
...
├── scripts/             # shell scripts the services poll or that binds call
...
| GPU-busy figure is blank | no `nvidia-smi` (ships with `cuda-devel`, not default) | expected; VRAM/temp/fan still come from `nvidia-settings` |
...
~/.cache/rd-shell/                    -- rendered matugen colours
```
